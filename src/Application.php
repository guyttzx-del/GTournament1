...
function is_admin_user(): bool { static $admin; if ($admin !== null) return $admin; if (!current_user() || !supabase()) return $admin = false; try { $rows = supabase()->rest('staff_roles', 'user_id=eq.' . rawurlencode((string) current_user()['id']) . '&role=eq.admin&limit=1'); return $admin = (bool) $rows; } catch (Throwable) { return $admin = false; } }
