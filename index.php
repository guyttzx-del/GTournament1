...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if ($action === 'signup') {
            (new AuthService(supabase()))->signUp((string) ($_POST['email'] ?? ''), (string) ($_POST['password'] ?? ''));
            flash('success', 'สมัครบัญชีแล้ว กรุณายืนยันอีเมลก่อนเข้าสู่ระบบ'); header('Location: ?view=auth'); exit;
        }
        if ($action === 'login') {
            establish_session((new AuthService(supabase()))->signIn((string) ($_POST['email'] ?? ''), (string) ($_POST['password'] ?? '')));
            header('Location: ?view=account'); exit;
        }
        if ($action === 'forgot-password') {
            (new AuthService(supabase()))->requestPasswordReset((string) ($_POST['email'] ?? ''));
            flash('success', 'หากอีเมลนี้มีบัญชี ระบบจะส่งลิงก์ตั้งรหัสผ่านใหม่ให้'); header('Location: ?view=auth'); exit;
        }
        ...
        if ($action === 'save_season') {
            ...
            $payload = ['name' => trim((string) ($_POST['name'] ?? '')), 'subtitle' => trim((string) ($_POST['subtitle'] ?? '')), 'status' => (string) ($_POST['status'] ?? 'draft'), 'capacity' => min(32, max(1, (int) ($_POST['capacity'] ?? 32))), 'entry_fee' => max(0, (float) ($_POST['entry_fee'] ?? 0)), 'registration_opens_at' => $_POST['registration_opens_at'] ?: null, 'registration_closes_at' => $_POST['registration_closes_at'] ?: null, 'promptpay_name' => trim((string) ($_POST['promptpay_name'] ?? '')), 'promptpay_number' => trim((string) ($_POST['promptpay_number'] ?? '')), 'expected_payment' => max(0, (float) ($_POST['expected_payment'] ?? 0))];
            if ($payload['name'] === '') throw new InvalidArgumentException('กรุณาระบุชื่อ Season');
            if ($seasonId !== '') supabase()->rest('seasons', 'id=eq.' . rawurlencode($seasonId), 'PATCH', $payload); else supabase()->rest('seasons', '', 'POST', [['code' => strtoupper('GT-' . bin2hex(random_bytes(3)))] + $payload]);
            supabase()->rest('audit_logs', '', 'POST', [['actor_id' => $user['id'], 'action' => 'season.saved', 'entity_type' => 'season', 'entity_id' => $seasonId ?: null, 'metadata' => $payload]]);
...
if ($staff && $appConfigured) { try { $staffStatus = $_GET['status'] ?? 'pending_review'; $staffStatus = in_array($staffStatus, ['pending_review','approved','rejected','all'], true) ? $staffStatus : 'pending_review'; $staffSearch = trim((string) ($_GET['search'] ?? '')); $staffQuery = $staffStatus === 'all' ? 'order=created_at.asc&limit=50' : 'status=eq.' . rawurlencode($staffStatus) . '&order=created_at.asc&limit=50'; $pendingRegistrations = supabase()->rest('registrations', $staffQuery); if ($staffSearch !== '') $pendingRegistrations = array_values(array_filter($pendingRegistrations, static fn(array $r): bool => stripos((string) ($r['nickname'] ?? '') . ' ' . (string) ($r['competition_name'] ?? ''), $staffSearch) !== false)); } catch (Throwable) { $pendingRegistrations = []; } }
$slipLinks = [];
...
$allowedViews = ['home', 'tournaments', 'rules', 'ranking', 'register', 'auth', 'account', 'staff', 'admin', 'privacy', 'terms', 'contact'];
if (!in_array($view, $allowedViews, true)) {
    $view = 'home';
}
if ($view === 'staff' && !$staff) http_response_code(403);
if ($view === 'admin' && !$admin) http_response_code(403);
...
<div class="site-shell">
    ...
    <?php elseif ($view === 'admin'): ?>
        <section class="page-hero section-pad"><p class="eyebrow">ADMIN / SEASON CONTROL</p><h1>จัดการ Season<br><em>และการรับสมัคร</em></h1><p>ตั้งค่าช่วงเวลา จำนวนผู้สมัคร ค่าสมัคร และ PromptPay จากศูนย์กลางทีมงาน</p></section><section class="section-pad content-section"><div class="register-layout"><form class="register-card" method="post"><input type="hidden" name="action" value="save_season"><?= csrf_field() ?><label>Season ID (เว้นว่างเมื่อสร้างใหม่)<input type="text" name="season_id"></label><label>ชื่อ Season<input type="text" name="name" required></label><label>คำอธิบาย<input type="text" name="subtitle"></label><label>สถานะ<select name="status"><option value="draft">ร่าง</option><option value="open">เปิดรับสมัคร</option><option value="closed">ปิดรับสมัคร</option><option value="running">กำลังแข่งขัน</option><option value="completed">จบแล้ว</option></select></label><label>Capacity<input type="number" name="capacity" min="1" max="32" value="32"></label><label>ค่าสมัคร<input type="number" name="entry_fee" min="0" step="0.01" value="25"></label><label>เปิดรับสมัคร<input type="datetime-local" name="registration_opens_at"></label><label>ปิดรับสมัคร<input type="datetime-local" name="registration_closes_at"></label><label>ชื่อบัญชี PromptPay<input type="text" name="promptpay_name"></label><label>หมายเลข PromptPay<input type="text" name="promptpay_number"></label><label>ยอดที่ต้องชำระ<input type="number" name="expected_payment" min="0" step="0.01" value="25"></label><button class="btn btn-primary btn-wide" type="submit">บันทึก Season</button></form><?php if (!$admin): ?><div class="register-card"><h2>ไม่มีสิทธิ์ Admin</h2><p>ต้องใช้บัญชี Admin เพื่อแก้ไขข้อมูล Season</p></div><?php endif; ?></div></section>
